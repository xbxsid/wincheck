# WinLite Security Check + CIS-style baseline + OS/Disk/SMART/Restore Storage → n8n (no HMAC)
...
# --- Helpers ---
function Add-Row { param([string]$Item,[string]$Status,[string]$Detail)
    if (-not $script:Rows) { $script:Rows = New-Object System.Collections.Generic.List[psobject] }
...
function YesNo([bool]$b) {
    if ($b) { return "Yes" } else { return "No" }
}

function Send-ToN8N {
    param([string]$Url,[object]$Rows,[int]$TimeoutSec,[int]$Retries,[int]$DelaySec)

    $payload = @{ rows = $Rows } | ConvertTo-Json -Depth 6
    for ($i=0; $i -le $Retries; $i++) {
        try {
            Invoke-RestMethod -Uri $Url -Method Post -Headers @{ "Content-Type" = "application/json" } -Body $payload -TimeoutSec $TimeoutSec | Out-Null
            Write-Host "✓ Sent results to n8n ($Url)"
            return $true
        } catch {
            Write-Warning "POST attempt $($i+1) failed: $($_.Exception.Message)"
            if ($i -lt $Retries) { Start-Sleep -Seconds $DelaySec }
        }
    }
    # Queue for later if all attempts fail
    $ts = (Get-Date).ToString("yyyyMMdd_HHmmss")
    $qPath = Join-Path $QueueDir "winlite_$($env:COMPUTERNAME)_$ts.json"
    $payload | Out-File -FilePath $qPath -Encoding UTF8
    Write-Warning "Queued payload for retry: $qPath"
    return $false
}
...
try {
    ...
    $inst = if ($os.InstallDate) { ([Management.ManagementDateTimeConverter]::ToDateTime($os.InstallDate)).ToString("yyyy-MM-dd") } else { "" }
    $lbt  = if ($os.LastBootUpTime) { ([Management.ManagementDateTimeConverter]::ToDateTime($os.LastBootUpTime)).ToString("yyyy-MM-dd HH:mm") } else { "" }
...
} catch { Add-Row "Current user in Local Administrators" "Unknown" "Check failed" }
...
try {
    ...
    if (-not $admins) {
        ...
        if ($raw) {
            $list = $raw | Select-Object -SkipWhile { $_ -notmatch '^-+$' } | Select-Object -Skip 1 |
...
} catch { Add-Row "Local Administrators group members" "Unknown" "Get-LocalGroupMember/net failed" }

# 2b) Built-in Administrator disabled (SID -500)
try {
    $adminName = $null
    $disabled  = $null
    ...
    if ($adminName) {
        $ok = ($disabled -eq $true)
        Add-Row "Built-in Administrator disabled" (YesNo $ok) ("Name=$adminName; Disabled=$disabled")
    } else {
...
try {
    ...
    if (Test-Cmd "Get-LocalUser") {
        ...
        if ($g) {
            $guestName = $g.Name
            $guestDis  = ($g.Enabled -eq $false)
        }
    }
    if ($guestName) {
        $ok = ($guestDis -eq $true)
        Add-Row "Guest account disabled" (YesNo $ok) ("Disabled=$guestDis")
    } else {
...
# 2d) Password & lockout policy (net accounts summary)
try {
...
# 3) Windows File Sharing (SMB server)
try {
    $serverSvc = Get-Service -Name "LanmanServer" -ErrorAction Stop
    $svcState  = $serverSvc.Status.ToString()
    $fwRules   = Get-NetFirewallRule -DisplayGroup "File and Printer Sharing" -ErrorAction SilentlyContinue | Where-Object {$_.Enabled -eq 'True'}
    $fwState   = if($fwRules){ "Enabled (" + ($fwRules | Select-Object -ExpandProperty Profile | Sort-Object -Unique -join ',') + ")" } else { "Disabled" }
    $status = if ($serverSvc.Status -eq 'Running') { "On" } else { "Off" }
    Add-Row "Windows File Sharing (SMB server)" $status ("Server service: $svcState; Firewall: $fwState")
} catch { Add-Row "Windows File Sharing (SMB server)" "Unknown" "Query failed" }

# 4) SMBv1 disabled
try {
    $feat = Get-WindowsOptionalFeature -Online -FeatureName SMB1Protocol -ErrorAction Stop
    $status = if ($feat.State -eq 'Disabled') { "Yes" } else { "No" }
    Add-Row "SMBv1 disabled" $status ("State=" + $feat.State)
} catch { Add-Row "SMBv1 disabled" "Unknown" "Get-WindowsOptionalFeature failed" }

# 5) Firewall enabled (Domain/Private/Public)
try {
    ...
    $status = if ($allOn) { "Yes" } else { "No" }
    Add-Row "Firewall enabled (Domain/Private/Public)" $status $detail
} catch { Add-Row "Firewall enabled (Domain/Private/Public)" "Unknown" "Get-NetFirewallProfile failed" }

# 5b) LLMNR disabled (CIS recommends disabled)
try {
    $val = (Get-ItemProperty "HKLM:\SOFTWARE\Policies\Microsoft\Windows NT\DNSClient" -ErrorAction SilentlyContinue).EnableMulticast
    $txt = if ($null -eq $val) { "Not set" } else { [string]$val }
    if ($null -eq $val) {
        Add-Row "LLMNR disabled" "Unknown" ("EnableMulticast=$txt")
    } else {
        $ok = ($val -eq 0)
        Add-Row "LLMNR disabled" (YesNo $ok) ("EnableMulticast=$txt")
    }
} catch {
...
try {
    ...
    $luaDesc = if ($enableLUA -eq 1) { "Enabled" } else { "Disabled" }
...
} catch { Add-Row 'UAC "Always Notify"' "Unknown" "Registry read failed" }

# 7) RDP disabled
try {
    $deny = (Get-ItemProperty "HKLM:\System\CurrentControlSet\Control\Terminal Server").fDenyTSConnections
    $status = if ($deny -eq 1) { "Yes" } else { "No" }
    Add-Row "RDP disabled" $status ("fDenyTSConnections=$deny")
} catch { Add-Row "RDP disabled" "Unknown" "Registry read failed" }
...
try {
    ...
    $hkcuOk = ($hkcu -eq 255)
    $status = "No"
    if ($hkmlOk -and $hkcuOk) { $status = "Yes" }
    elseif ($hkmlOk -or $hkcuOk) { $status = "Partial" }
    $detail = "HKLM=$hkml; HKCU=$hkcu (expect 255)"
    Add-Row "AutoPlay/AutoRun disabled" $status $detail
} catch { Add-Row "AutoPlay/AutoRun disabled" "Unknown" "Registry read failed" }

# 8b) Audit policy (summary)
try {
    $audit = auditpol /get /category:* 2>$null
    if ($audit) {
        $snippet = ($audit | Select-Object -First 20) -join " | "
        Add-Row "Audit policy (summary)" "Info" ("auditpol sample: " + $snippet)
    } else {
...
# 9) System Restore (enabled + storage on C:)
try {
    ...
    $status = if ($enabled) { "Yes" } else { "No" }
    Add-Row "System Restore enabled (C:)" $status ("DisableSR=$disableSR")

    $shadow = Get-CimInstance -ClassName Win32_ShadowStorage -ErrorAction SilentlyContinue | Where-Object { $_.Volume -match "C:" }
    if ($shadow) {
        $alloc = [uint64]$shadow.AllocatedSpace
        $max   = [uint64]$shadow.MaximumSpace
        $used  = [uint64]$shadow.UsedSpace
        $pct   = if ($max -gt 0) { [math]::Round(($alloc / $max) * 100, 2) } else { 0 }
        $detail = "Allocated=$([Format-Bytes] $alloc); Maximum=$([Format-Bytes] $max); Used=$([Format-Bytes] $used); Alloc%=$pct"
...
# 10) BitLocker enabled (C:)
try {
    $bl = Get-BitLockerVolume -MountPoint "C:" -ErrorAction SilentlyContinue
    if ($bl) {
        $prot = $bl.ProtectionStatus
        $status = if ($prot -eq 'On') { "Yes" } else { "No" }
        Add-Row "BitLocker enabled (C:)" $status ("ProtectionStatus=$prot")
    } else {
        Add-Row "BitLocker enabled (C:)" "Unknown" "BitLocker cmdlet not available or no volume info"
    }
} catch { Add-Row "BitLocker enabled (C:)" "Unknown" "Query failed" }

# =============================
#       DISK & SMART
# =============================

# 11) Logical disk storage (all fixed drives)
try {
    ...
    foreach ($d in $ldisks) {
        ...
        $used = ($size - $free)
        $pctFree = if ($size -gt 0) { [math]::Round(($free / $size) * 100, 2) } else { 0 }
        $status = if ($pctFree -ge $MinFreePercent) { "Yes" } else { "No" }
        $detail = "Drive=$($d.DeviceID); Total=$([Format-Bytes] $size); Free=$([Format-Bytes] $free); Used=$([Format-Bytes] $used); %Free=$pctFree"
...
# 12) Physical disk SMART health
$smartReported = $false
try {
    $pds = Get-PhysicalDisk -ErrorAction Stop
    foreach ($pd in $pds) {
        $hs = $pd.HealthStatus  # Healthy/Warning/Unhealthy/Unknown
        $status = if ($hs -eq 'Healthy') { "Yes" } elseif ($hs -eq 'Unknown') { "Unknown" } else { "No" }
        $detail = "FriendlyName=$($pd.FriendlyName); MediaType=$($pd.MediaType); HealthStatus=$hs; Size=$([Format-Bytes]([uint64]$pd.Size))"
    ...
    try {
        ...
        foreach ($p in $pred) {
            ...
            if ($p.InstanceName -and $drives) {
                foreach ($d in $drives) {
                    if ($p.InstanceName -like ("*" + $d.SerialNumber + "*") -or $p.InstanceName -like ("*" + $d.Model + "*")) {
                        $model = "$($d.Model) SN=$($d.SerialNumber)"
                        break
                    }
                }
            }
            $status = if ($predictFail -eq $true) { "No" } else { "Yes" }
            $detail = "PredictFailure=$predictFail; Disk=$([string]::IsNullOrEmpty($model) ? $p.InstanceName : $model)"
...
# 13) Physical disk TYPE (SSD / HDD)
$diskTypeReported = $false
try {
    $pds2 = Get-PhysicalDisk -ErrorAction Stop
    foreach ($pd in $pds2) {
        $media = if ($pd.MediaType) { $pd.MediaType.ToString() } else { "Unknown" }
        $detail = "FriendlyName=$($pd.FriendlyName); MediaType=$media; BusType=$($pd.BusType); Size=$([Format-Bytes]([uint64]$pd.Size))"
...
if (-not $diskTypeReported) {
    try {
        ...
        foreach ($d in $msft) {
            ...
            $detail = "FriendlyName=$($d.FriendlyName); MediaType=$mt; Size=$([Format-Bytes]([uint64]$d.Size))"
            Add-Row "Physical disk type (SSD/HDD)" "Info" $detail
        }
        $diskTypeReported = $true
    } catch { }
    ...
    try {
        ...
        foreach ($wd in $wmiDisks) {
            ...
            if ($wd.RotationRate -ne $null) {
                ...
                if ($rot -gt 0) { $mediaGuess = "HDD ($rot RPM)" } else { $mediaGuess = "SSD/Unknown (0 RPM)" }
            }
            $detail = "Model=$($wd.Model); MediaGuess=$mediaGuess; Size=$([Format-Bytes]([uint64]$wd.Size))"
            Add-Row "Physical disk type (SSD/HDD)" "Info" $detail
        }
    } catch {
...
Write-Host "`nCSV report: $CsvPath"

# Prepare rows to send / compliance evaluation
$rowsToSend = if ($OnlySendNonCompliant) { $Rows | Where-Object { $_.status -eq "No" } } else { $Rows }
$nonCompliantCount = ($Rows | Where-Object { $_.status -eq "No" }).Count

# Send to n8n
if (-not $WebhookUrl -or $WebhookUrl -eq "https://n8n.comguys.co/webhook/win") {
    Write-Warning "WebhookUrl not set or left as default. Skipping POST."
} elseif ($rowsToSend.Count -gt 0) {
...
if ($FailOnNonCompliant -and $nonCompliantCount -gt 0) {
    Write-Warning "Non-compliant checks: $nonCompliantCount"
    exit 2
}
exit 0
